...
/**
 * Test component to verify Firestore real-time sync
 * Displays a simple counter that syncs across all connected clients
 */
export default function TestSync() {
  ...
  useEffect(() => {
    // Set up real-time listener for test collection
    const q = query(
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'test-sync'), {
        ...
        client: Math.random().toString(36).substring(7) // Random client ID
      });
      setInputText('');
    } catch (err) {
